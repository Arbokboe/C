...
int s21_sprintf(char *buff, char *format, ...) {
  *buff = 0;
  ...
  flags_parse flags = {0};
  for (; *format; format++) {
    s21_parser(buff, factor, &format, &flags);
  }
  va_end(factor);
  sum = s21_strlen(buff);
  return sum;
...
void s21_parser(char *buff, va_list factor, char **c_form, flags_parse *flags) {
  ...
  int flag_point = 0;
  if (**c_form != '%') {
    s21_processing_char(**c_form, buff);
  } else {
    flag_start = 1;
    s21_flag_reset(flags);
    (*c_form)++;
  }
  while (flag_start) {
    s21_parser_flags(c_form, flags);
    s21_NumbersParser(c_form, flags, flag_point);
    if (**c_form == '.') {
      flag_point = 1;
      (*c_form)++;
      s21_NumbersParser(c_form, flags, flag_point);
    }
    s21_LengthParser(c_form, flags);
    s21_parser_spec(&flag_start, c_form, flags);
    if (flag_start) {
      (*c_form)++;
    } else {
      s21_processing_all(factor, flags, buff, c_form);
    }
  }
}
...
void s21_processing_all(va_list factor, flags_parse *flags, char *buff,
  ...
  if (flags->spec_percent) {
    s21_processing_spec_percent(flags, buff);
  }
  if (flags->spec_c) {
    s21_processing_spec_c(factor, flags, buff);
  }
  if (flags->spec_s) {
    s21_processing_spec_s(factor, flags, buff);
  }
  if (flags->spec_d == 1) {
    s21_processing_spec_d(factor, buff, flags);
  }
  if (flags->spec_u) {
    s21_processing_spec_d(factor, buff, flags);
  }
  if (flags->spec_f == 1) {
    ...
    s21_processing_spec_f(f, buff, flags);
  }
  if (flags->spec_o == 1) {
    long long int o = va_arg(factor, long long);
    s21_processing_spec_o(o, buff, flags);
  }
  if (flags->spec_x || flags->spec_X == 1) {
    long long int x = va_arg(factor, long long);
    s21_processing_spec_x(x, buff, c_form, flags);
  }
}
...
void s21_processing_spec_f(double f, char *buff, flags_parse *flags) {
  ...
  s21_spec_f_width(aggregate_str, &add_char, &len_sub_buff, sub_buff, flags);
  if (flags->fl_space && !flags->negative && !flags->fl_plus)
    add_space(sub_buff);
  s21_strcat(buff, sub_buff);
}
    ...
    *add_char = flags->desc_width - (*len_sub_buff);
    ...
    if (!flags->fl_minus) {
      ...
      for (int i = 0; i < *add_char; i++) {
        s21_strcat(zero_line, aggregate_str);
      }
      s21_strcat(zero_line, sub_buff);
      sub_buff[0] = '\0';
      s21_strcat(sub_buff, zero_line);
...
void add_minus(char *sub_buff) {
  ...
  point_minus[0] = '-';
  point_minus[1] = '\0';
  s21_strcat(point_minus, sub_buff);
  sub_buff[0] = '\0';
  s21_strcat(sub_buff, point_minus);
}
...
void add_plus(char *sub_buff) {
  ...
  point_plus[0] = '+';
  point_plus[1] = '\0';
  s21_strcat(point_plus, sub_buff);
  sub_buff[0] = '\0';
  s21_strcat(sub_buff, point_plus);
}
...
void add_space(char *sub_buff) {
  ...
  point_space[0] = ' ';
  point_space[1] = '\0';
  s21_strcat(point_space, sub_buff);
  sub_buff[0] = '\0';
  s21_strcat(sub_buff, point_space);
}
...
void floatToString(double f, char *sub_buff, flags_parse *flags) {
  ...
  s21_strcat(str_all, str_whole);
  s21_floatToString_no_precision_zero(str_fractional, str_all, &zero_point,
                                      flags);
  s21_strcat(sub_buff, str_all);
}
...
void s21_floatToString_no_precision_zero(char *str_fractional, char *str_all,
  ...
  if (!flags->desc_precision_zero) {
    ...
    char point_zero[] = "N";
    point_zero[0] = '0';
    s21_strcat(str_all, point_c);
    if (*zero_point > 0) {
      for (int i = 0; i < *zero_point; i++) {
        s21_strcat(str_all, point_zero);
      }
    }
    s21_strcat(str_all, str_fractional);
  }
}
...
void s21_processing_spec_s(va_list factor, flags_parse *flags, char *buff) {
  ...
  if (!jopa) {
    ...
    if (jopa) {
      srat = 1;
      s21_strcat(jopa, "(null)");
    }
  }
  char s[s21_strlen(jopa)];
  if (flags->desc_precision || flags->desc_precision_zero) {
    if (flags->desc_precision < (int)s21_strlen(jopa)) {
...
void s21_processing_spec_percent(flags_parse *flags, char *buff) {
  ...
  percent[0] = '%';
  percent[1] = '\0';
  s21_print_str(flags, percent, buff);
}
...
void s21_print_str(flags_parse *flags, char *str, char *buff) {
  ...
  int start = flags->desc_width - s21_strlen(str);
  if (start >= 0) {
    if (flags->desc_width) {
      if (flags->fl_minus) {
        for (size_t i = 0; i < flags->desc_width - s21_strlen(str); i++) {
          if (i == 0) {
            s21_strcat(buff, str);
          }
          s21_strcat(buff, " ");
...
void s21_processing_spec_o(long long int o, char *buff, flags_parse *flags) {
  ...
  s21_format_output(str, buff, flags);
}
...
void s21_format_output(char *str, char *buff, flags_parse *flags) {
  ...
  if (str && str2 && str3) {
    ...
    if (!flags->fl_minus) {
      ...
      if (flags->desc_width > (int)s21_strlen(str2)) {
        flags->desc_width -= (int)s21_strlen(str2);
      } else {
        flags->desc_width = 0;
      }
    ...
    if (flags->fl_minus) {
      ...
      s21_strcat(str2, str);
      flags->desc_width = (flags->desc_width > (int)s21_strlen(str2))
                              ? flags->desc_width - s21_strlen(str2)
                              : 0;
      for (int i = s21_strlen(str2), j = 0; j < flags->desc_width; i++, j++)
        str2[i] = ' ';
      s21_strcat(buff, str2);
    }
